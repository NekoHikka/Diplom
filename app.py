...
def accept_invite(id):
    ...
    if p and p.user2_id == current_user.id:
        ...
        if not existing_categories:
            cats = [('🛒 Супермаркет', 'Витрата'), ('🍔 Ресторани/Кафе', 'Витрата'), ('🏠 Оренда/Житло', 'Витрата'), ('⚡ Комунальні', 'Витрата'), ('🚗 Авто/Транспорт', 'Витрата'), ('💊 Аптека/Здоров\'я', 'Витрата'), ('🍿 Розваги', 'Витрата'), ('🐶 Улюбленці', 'Витрата'), ('💰 Загальний Дохід', 'Дохід')]
...
def add_goal():
    ...
    db.session.add(Goal(name=request.form['name'], target_amount=target_amount, account_ids=acc_str, user_id=uid, is_shared=is_shared))
    db.session.commit()
    return redirect(url_for('shared_budget' if is_shared else 'home'))
...
def add_receipt_ai():
    ...
    try:
        ...
        Приклад правильної відповіді (для чека з магазину):
        [
          {{
            "type": "Витрата",
            "amount": 345.50,
            "category": "🛒 Супермаркет",
            "description": "Супермаркет КЛАСС",
            "date": "2026-03-18"
          }}
        ]
        """
        ...
        if acc and isinstance(transactions_data, list) and len(transactions_data) > 0:
            ...
            for td in transactions_data:
                ...
                if cat_name not in known_cat_names:
                    new_cat = Category(name=cat_name, type=t_type, user_id=current_user.id, is_shared=is_shared)
                ...
                new_t = Transaction(
                    user_id=current_user.id, account_id=acc.id, type=t_type, category=cat_name,
                    amount=amount, date=t_date, description=td.get('description', 'Розпізнано ШІ 🤖'), is_shared=is_shared
                )
            ...
            if statement_resp.status_code == 200:
                ...
                for t in transactions:
                    ...
                    existing = Transaction.query.filter_by(
                        user_id=current_user.id, account_id=account_db_id,
                        amount=amount_uah, type=t_type, description=t_desc, date=t_date
                    ).first()
                    
                    if not existing:
                        new_tx = Transaction(
                            user_id=current_user.id, account_id=account_db_id, type=t_type,
                            category='Інше', amount=amount_uah, date=t_date, description=t_desc, is_shared=False
                        )
...
def analytics():
    ...
    if is_shared and has_partner:
        ...
        if is_shared and not has_partner:
            is_shared = False 
    ...
    if is_shared and has_partner:
        older_expenses = Transaction.query.filter(Transaction.user_id.in_(user_ids), Transaction.is_shared==True, Transaction.type=='Витрата', Transaction.date >= previous_start_date, Transaction.date < start_date).all()
    else:
        older_expenses = Transaction.query.filter(Transaction.user_id == current_user.id, Transaction.is_shared==False, Transaction.type=='Витрата', Transaction.date >= previous_start_date, Transaction.date < start_date).all()
    ...
    if older_sum > 0:
        if total_expense > (older_sum * 1.1):
            trend_msg = f"⚠️ Витрати зросли на {int(((total_expense / older_sum) - 1) * 100)}% (порівняно з минулими {period_days} дн.)!"
            trend_color = "#ff4d4d"
        elif total_expense < (older_sum * 0.9):
            trend_msg = f"✅ Витрати впали на {int((1 - (total_expense / older_sum)) * 100)}% (порівняно з минулими {period_days} дн.)"
            trend_color = "#4CAF50"
        else:
            trend_msg = "⚖️ Витрати стабільні."
            trend_color = "#aaa"
    else:
        if total_expense > 0:
            trend_msg = "📊 Немає даних за попередній період для порівняння."
            trend_color = "#aaa"
