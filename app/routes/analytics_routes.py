...
from app.repositories.category_repository import get_categories_by_user, get_multi_user_categories
...
def export():
    ...
    output = ExportService.export_to_excel(filtered_tx, filter_type)
...
def analytics():
    ...
    if is_shared and has_partner:
        user_ids = [current_user.id, partner.user1_id if partner.user2_id == current_user.id else partner.user2_id]
        expenses = [t for t in get_shared_transactions(user_ids, start_date) if t.type == 'Витрата']
        ...
        expenses = [t for t in get_user_transactions(current_user.id, start_date) if t.type == 'Витрата']
    ...
    for exp in expenses:
        ...
        translated_cat = translate_name(cat)
    ...
    if is_shared and has_partner: older_expenses = [t for t in get_shared_transactions(user_ids, previous_start_date) if t.date < start_date and t.type == 'Витрата']
    else: older_expenses = [t for t in get_user_transactions(current_user.id, previous_start_date) if t.date < start_date and t.type == 'Витрата']
    ...
    if not recommendations and total_expense > 0: recommendations.append(get_string('rec_balanced'))
    ...
    chart_colors = [cat_color_map.get(label, random.choice(Config.COLORS_PALETTE)) for label in list(category_totals.keys())]
    ...
    return render_template('analytics.html', period_days=period_days, top_category=top_category, 
                           ...
                           labels=list(category_totals.keys()), values=list(category_totals.values()), 
